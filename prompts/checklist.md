...
2. 줄바꿈 수는 영문 이상이다. 적으면 문단 누락이므로 영문을 문단 단위로 대조해 빠진 문단만 추가한다 (missing_paragraph).
   - 텍스트에셋 줄은 flag 유무와 무관하게 영문과 **문장 단위**로 대조한다. 영문의 모든 문장에 대응하는 한국어 문장이 있어야 하며, 문단 수가 같아도 빠진 문장이 있으면 목표 문체로 보충한다. 원문에 없는 문장은 삭제하거나 해당 영문 문장의 번역으로 대체한다.
   - register_table.md 6.3의 길이 비율(영문의 0.42배) 기준은 참고용일 뿐 판정 근거가 아니다. 비율이 맞아도 문장이 빠져 있을 수 있다.
...
4. 영문 효과 표기 `[+1 Authority]`가 남지 않는다 (english_effect_tag).
   - 효과 표기의 라벨은 게임 HUD 스탯 명칭을 따른다(R72): 권위, 예산, 에너지, 군수 장비, 군사 인력, 전차, 지원 차량, 폭격기, 전투기, 군함, 잠수함. "군사 장비"·"장비"→"군수 장비", "인력"·"병력"→"군사 인력", "함선"→"군함"으로 쓴다. 기계 처리가 이미 통일했으므로 새로 쓰는 문장에서만 이 표기를 지키면 된다. 부대 단위(보병 사단, 산악부대 등)와 "턴당 N …"은 그대로 둔다.
...
5. 용어는 glossary.md의 표준 표기만 쓴다. 용어 표기는 glossary.md가 최종 기준이며 register_table.md 6.4의 후보 목록과 어긋나면 glossary.md를 따른다 (glossary_violation).
   ...
   - glossary.md에 없는 용어·고유명사는 이 코퍼스(`data/current/dialogue.jsonl`, `data/current/textassets.jsonl`)에서 이미 다수로 쓰이는 표기로 통일할 수 있다(예: "RRN 발레로" 23줄 ← 로마자 "RRN Valero" 6줄, "팔란토르" 44줄 ← 로마자 "Palantor" 3줄, "소르들란드" ← 오타 "소르드"). 예시의 줄 수는 배치가 반영되면서 바뀌므로 그대로 믿지 말고 직접 센다. 근거 수치(`Grep -c`로 센 줄 수)를 reason에 적는다. 코퍼스에 없는 새 표기를 만드는 것은 여전히 금지다.
   - 고유명(예: 리치아 정보국, RIS)과 보통명사(예: 대외정보기관)는 문맥에 따라 구분해 쓴다. 다만 둘 다 코퍼스에 이미 있는 표기여야 한다.
   ...
   - (R94) 루시타 아자로는 본인 작위자이므로 "아자로 여공작"/"여공작"(기계 치환됨). "공작부인"은 공작의 아내에게만 쓴다. 왕비가 된 뒤에는 "루시타 왕비"(R89).
   - (R93) 로마자 약칭(MITZ·ITZ·RRG·RNC·RPP·ATO·CSP·GRACE·ANDOR·HOD 등)은 메인 캠페인 관례대로 로마자 그대로 둔다. R86은 약칭이 아닌 브랜드·사업명에만 적용한다. 단 본문에서 풀어 쓴 정식 명칭은 용어집 표준(메프티엠 국제무역지대 등)을 따른다.
   - (R86) 브랜드·사업명처럼 코퍼스에 **한글 표기 선례가 있는** 고유명은 로마자 잔존 줄이 더 많아도 한글 다수형으로 통일한다(예: PaleStream → 페일스트림). 로마자 잔존은 미번역 흔적이지 표기 선택이 아니다.
   ...
   - (R78~R81) 확정 표기: decree→왕령(UI 패널 라벨 기준, 칙령 금지), 비나→베아트리체는 "베아 이모"(레나의 언니), Captain Gordion→고르디온 대장(직위; 대위 금지), noble houses→귀족 가문(들)(왕가는 토라스 하나뿐). 모두 기계 치환됨.
...
12. 로무스의 해요체는 상대가 에스텔라일 때만 정당하다. 그 외에는 register_table.md 5.1의 상대별 화계로 바꾼다 (romus_haeyo).
    - 주의(R99, 루시타 혼인 후): 아버지·사촌 등 타인이 동석한 장면은 공적 → 로무스 하오체+"그대", 루시타 합쇼체+"폐하". 단둘임이 나레이션으로 분명한 줄만 양쪽 모두 해체(로무스도 반말+"당신"). 한쪽만 해체인 비대칭은 금지.
    - 주의(R83, 휴고 사석 대사): 휴고→로무스 사석 하오체는 **대사 단위**로 적용한다. 플레이어 선택이 끼지 않은 연속 노드는 앞 노드의 화계를 잇는다(호격이 없는 뒷줄이라고 합쇼체로 바꾸지 않는다). 안젤리카의 칼레카비즈 유배는 "유배"(R84; 추방·망명 금지).
    - 주의(R71/R71b, 가족이 여럿인 장면): 화계는 그 대사의 **실제 청자**를 따른다. 에스텔라가 자리에 있어도 휴고에게 분명히 하는 말은 하오체다. 여럿에게 하는 말이거나 청자가 불분명하면 그 자리의 최상위 인물을 기준으로 한다(어머니가 있으면 해요체, 없으면 하오체). 한 선택지 노드 안에서 화계를 바꾸지 않는다.
...
14. 신하는 합쇼체, 비나는 해요체, 외국 정상은 합쇼체, 휴고는 하게체·해라체 금지 (subject_not_hapsyo, vina_not_haeyo, foreign_not_hapsyo, hugo_low_register).
    ...
    - 주의(R95): 외국 정상·신하의 1인칭은 "저/제"다. 예외는 헤겔(적대적 외국 정상, 화계표 5.4)만 "나/내". 악셀·알바레츠·스몰라크·라이네 등은 "저/제"로 통일한다.
...
19. 유형별 목표 문체는 register_table.md 6.1을 따른다 (ta_register_mismatch).
    - 주의: `target`은 필드 유형으로 추정한 값이다. Title·Name처럼 문장이 아닌 필드(제목·이름·라벨)는 종결어미가 없으므로 `target`과 어긋나도 위반이 아니다.
