...
- 반영 검증기의 한계를 지킨다. 어기면 그 수정만 거절되어 `{BATCH_DIR}/apply_errors.jsonl`에 남는다: 수정문은 원문 한글 길이의 2배 이하, 영문이 있으면 영문 글자 수의 0.42배 이상; 줄바꿈 수는 원문보다 줄이지 않는다; `[…]` 효과 표기 개수는 원문과 같다; `{…}` 변수 집합은 영문과 같다.
